...
function creaRiga(e) {
    ...
    tdBtn3.addEventListener("click", function () {
        ...
        document.getElementById("aggiungi").classList.remove("active");
        console.log(valid)
        if (e.id == modId && validator == true && valid == true) {
            modifica.classList.remove("active");
            modId = "";
            validator = false;
            valid = false;
...
class Pagination {
    ...
    getVisibleItems(lista = this.items) {
        ...
        this.itemsToDisplay.forEach((e) => { creaRiga(e)})
    ...
    ricerca() {
        ...
        let filter = this.items.filter(item => { return item.username.toLowerCase().includes(searchInput.value)});
